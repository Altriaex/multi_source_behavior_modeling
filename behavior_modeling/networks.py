...
class ActorNetwork(tf.Module):
	...
	def create_action_distribution(self, mean, std):
		a_distribution = tfd.TransformedDistribution(
				distribution=tfd.MultivariateNormalDiag(loc=[0.] * mean.shape[-1], scale_diag=[1.] * mean.shape[-1],allow_nan_stats=False),

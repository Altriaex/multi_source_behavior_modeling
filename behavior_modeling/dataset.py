# coding=utf-8
# Adapted from https://github.com/Farama-Foundation/D4RL-Evaluations/blob/master/brac/behavior_regularized_offline_rl/brac/dataset.py
...
Transition_ = collections.namedtuple(
		'Transition', 's1, s2, a1, a2, discount, reward, tid')
...
class Dataset(tf.Module):
	...
	def __init__(
		...
		if has_tid:
			self._tid = self._zeros([size], tf.int32)
			self._data = Transition_(
					s1=self._s1, s2=self._s2, a1=self._a1, a2=self._a2,
					discount=self._discount, reward=self._reward, tid=self._tid)
		else:
